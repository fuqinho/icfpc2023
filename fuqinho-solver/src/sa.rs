...
fn move_at_random_pos(board: &mut Board, m: usize, rng: &mut ThreadRng) {
...
fn move_at_random_direction(board: &mut Board, m: usize, rng: &mut ThreadRng) {
...
fn move_at_gradient_direction(board: &mut Board, m: usize, rng: &mut ThreadRng) {
    let m_pos = board.musicians()[m].unwrap().0;
...
fn collide_at_random_direction(board: &mut Board, m: usize, rng: &mut ThreadRng) {
    let pos = board.musicians()[m].unwrap().0;
    ...
    loop {
        ...
        if rng.gen_range(0..10) == 0 {
            ...
            if r == 0 {
                ...
                move_at_random_pos(&mut board, m, &mut rng);
            } else if r == 1 {
                // 90*10 = 9%: Move random direction for 40 units at max.
                collide_at_random_direction(&mut board, m, &mut rng);
            } else if r == 2 {
                // 90*10 = 9%: Move to gradient direction for 40 units at max.
                move_at_gradient_direction(&mut board, m, &mut rng);
            } else {
                // 90*70 = 63%: Move a musician at random direction for up to 40 units.
                move_at_random_direction(&mut board, m, &mut rng);
            }

            // Revert the state if collides.
            let score = board.score_ignore_negative();
            if score > best_score {
                best_score = score;
            } else {
        ...
        eprintln!("iteration: {} score: {}", iteration, best_score);
        if iteration >= 100000 {
            break;
        }
    }
